...
{
    sfTime t = sfClock_getElapsedTime(global->mentor[0].clock);
    float sec = t.microseconds / 1000000.0;
    if (global->mentor[0].one == 1)
        global->mentor[0].tmp = global->mentor[0].tmpsec;
    global->mentor[0].one = 2;
    // if (global->bubble[0].sec > 17 && global->bubble[0].i == 3) {
    if (global->mentor[0].tmpsec < global->mentor[0].tmp + 5 && global->bubble[0].i == 3) {
