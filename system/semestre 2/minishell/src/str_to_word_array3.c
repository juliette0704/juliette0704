...
int calc_space3(int word_size, char *chaine, int new_count)
{
...
int my_cond3(char *chaine, int count)
{
    if (chaine[count] == '/')
        count++;
    return count;
...
char **str_to_word_array3(char *chaine)
{
    ...
    while (chaine[count] != '\0') {
        ...
        int new_count = count;
        int word_size = 0;
        word_size = calc_space3(word_size, chaine, new_count);
